...
setInterval(function () {
  ...
  if (
    (can_use("hp") && character.hp / character.max_hp <= 0.5) ||
  ...
  if (
    (can_use("mp") && character.mp / character.max_mp <= 0.5) ||
  ...
  if (target) {
    if (can_attack(target)) {
      ...
      let dist = Math.sqrt(
        Math.pow(target.real_x - character.real_x, 2) +
          Math.pow(target.real_y - character.real_y, 2)
      );
